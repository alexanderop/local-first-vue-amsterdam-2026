...
interface TimelineItem {
  ...
  description?: string
  details?: string[]
  click?: number              // reveal at this click (omit = always visible)
...
- Use `layout: center` with `class: flex items-center justify-center h-full` for full-slide diagrams with no other text
- Start click values at 1 (not 0) so the slide initially shows only the base diagram (e.g. the timeline arrow)
- The `variant` prop controls color theming per item (pink accent, green success, dimmed muted)
